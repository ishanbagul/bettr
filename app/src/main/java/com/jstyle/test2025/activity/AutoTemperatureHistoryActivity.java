...
public class AutoTemperatureHistoryActivity extends BaseActivity {
    ...
    public void dataCallback(Map<String, Object> maps) {
        ...
        switch (dataType){
            case BleConst.deleteGetTemperature_historyDataWithMode://删除设备自动测试出的温度数据 Delete the temperature data automatically tested by the device
                list.add(maps.toString());
                activityModeDataAdapter.setData(list);
                break;
            case BleConst.Temperature_history://读取设备自动测试出的温度数据 Read the temperature data automatically tested by the device
                list.add(maps.toString());
                dataCount++;
                if(finish){
                    dataCount=0;
